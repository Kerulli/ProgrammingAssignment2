## Rikaad Laloo

## Matrix inversion is usually a costly computation and 
##there may be some benefit to caching the inverse of a 
##matrix rather than compute it repeatedly.

##The Two functions below will cache the inverse of a matrix

## makeCacheMatrix is a function that creates a special 
## matrix object that can cache its inverse. It is made
## up of four functions, set, get, setmean, getmean.

## 1. set the value of the matrix by changing the 
## matrix stored in the main function

## 2. get the value of the matrix stored in the main function

## 3. setinverse sets the value of the inverse of the matrix

## 4. getinverse gets the value of the inverse of the matrix 

makeCacheMatrix <- function(x = matrix()) {
  
  inv <- NULL
  set <- function(y){
    x <<- y
    inv <<- NULL
  }
  get <- function() x
  setinverse <- function(inverse) inv <<- inverse
  getinverse <- function() inv
  
  list(set = set, get = get,
       setinverse = setinverse,
       getinverse = getinverse)

}


## cacheSolve computes the inverse of the special matrix
## returned by makeCacheMatrix above. If the matrix has already 
## been calculated and it has not changed, then 
## cacheSolve should retrieve the inverse from the 
## cache

## The function assumes the matrix is invertible. 


cacheSolve <- function(x, ...) {
        ## Return a matrix that is the inverse of 'x'
  inv <- x$getinverse()
  if(!is.null(inv)) {
    message("getting cached data")
    return(inv)
  }
  data <- x$get()
  inv <- solve(data, ...)
  x$setinverse(inv)
  inv
}
